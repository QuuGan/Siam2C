from models.Siam2C import SiamMask
...
class ResDown(MultiStageFeature):
    def __init__(self, pretrain=False):
        ...
        self.unfix(0.0)
        self.sa = SA(in_channels=256)
        self.ca = CA(in_dim=256)
        self.conv = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1)
        self.bn = nn.BatchNorm2d(256)
        self.relu = nn.ReLU()
        self.cbr = nn.Sequential(self.conv, self.bn, self.relu,nn.Conv2d(256, 256, 3, 2, 1))
    ...
    def forward_all(self, x):
        ...
        xs = self.sa(output[1])
        xc = self.ca(output[1])
        # z  = self.relu(self.bn(self.conv(xs +xc)))
        z = self.cbr(xs +xc)
        p3 = self.downsample(output[-1])
        p3 += z      
