'''
The current code is improved based on SiamMask (@inproceedings{wang2019fast,
    title={Fast online object tracking and segmentation: A unifying approach},
    author={Wang, Qiang and Zhang, Li and Bertinetto, Luca and Hu, Weiming and Torr, Philip HS},
    booktitle={Proceedings of the IEEE conference on computer vision and pattern recognition},
    year={2019}}).
 We introduced classification-rank loss.
'''
import argparse
import logging
import os
...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    for iter, input in enumerate(train_loader):
        ...
        rpn_cls_loss, CR_loss, rpn_loc_loss, rpn_mask_loss = torch.mean(outputs['losses'][0]), torch.mean(outputs['losses'][1]), torch.mean(outputs['losses'][2]), torch.mean(outputs['losses'][3])
        ...
        loss = rpn_cls_loss * cls_weight + CR_loss*0.5 + rpn_loc_loss * reg_weight + rpn_mask_loss * mask_weight
        ...
        avg.update(batch_time=batch_time, rpn_cls_loss=rpn_cls_loss, CR_loss=CR_loss, rpn_loc_loss=rpn_loc_loss,
        ...
        tb_writer.add_scalar('loss/Rank_loss_cls', CR_loss, tb_index)
        ...
        if (iter + 1) % args.print_freq == 0:
            logger.info('Epoch: [{0}][{1}/{2}] lr: {lr:.6f}\t{batch_time:s}\t{data_time:s}'
                        '\t{rpn_cls_loss:s}\t{CR_loss:s}\t{rpn_loc_loss:s}\t{rpn_mask_loss:s}\t{siammask_loss:s}'
                        ...
                        data_time=avg.data_time, rpn_cls_loss=avg.rpn_cls_loss, 
                        CR_loss=avg.CR_loss,rpn_loc_loss=avg.rpn_loc_loss,
